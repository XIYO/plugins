...
|---|---|---|---|
| `message-pipeline` | `message-pipeline` 하나 | KakaoTalk, iMessage | 로컬 메시지를 읽기 전용으로 추출하고 토큰 절약형 CCT로 변환한다. |
...
플러그인 설치는 단일 `message-pipeline` 스킬을 등록한다. `msgpipe` 실행 파일이 없으면 스킬이 번들된 `scripts/install-runtime.sh`를 사용해 Rust 앱을 별도로 설치한다. 사용자 데이터에 접근하는 `kakaocli`와 `imsg`는 자동 설치하지 않으며, 같은 스킬의 소스별 참조 문서가 진단·검증 절차를 제공한다.
