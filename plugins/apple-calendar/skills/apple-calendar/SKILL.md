...
```bash
CALCTL="$(command -v calctl || true)"
CALMETA="$(command -v calmeta || true)"
test -x "$CALCTL" && test -x "$CALMETA"
"$CALCTL" doctor
```

- 명령이 없으면 이 스킬이 속한 플러그인 루트의 `scripts/install-runtime.sh`를 실행한다. 플러그인 루트는 이 `SKILL.md`에서 두 디렉터리 위이며, 기본 설치 경로는 `~/.local/bin`이다.
