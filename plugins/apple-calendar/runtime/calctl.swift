...
private let calctlVersion = "0.1.2"

private enum CalctlError: Error, CustomStringConvertible {
    case usage(String)
...
private enum Logger {
    static let threshold = LogLevel.parse(ProcessInfo.processInfo.environment["CALCTL_LOG_LEVEL"])
...
private struct Arguments {
    ...
    mutating func option(_ name: String) throws -> String? {
        ...
        guard valueIndex < values.endIndex else {
            throw CalctlError.usage("\(name) requires a value")
        }
    ...
    mutating func positional(_ label: String) throws -> String {
        guard let index = values.firstIndex(where: { !$0.hasPrefix("--") }) else {
            throw CalctlError.usage("missing \(label)")
        }
    ...
    func finish() throws {
        guard values.isEmpty else {
            throw CalctlError.usage("unknown arguments: \(values.joined(separator: " "))")
        }
    }
}
    ...
    func object(_ value: Any) throws {
        if json {
            ...
            guard let text = String(data: data, encoding: .utf8) else {
                throw CalctlError.operation("failed to encode JSON output")
            }
            print(text)
...
private enum Dates {
    ...
    static func day(_ value: String, endOfDay: Bool = false) throws -> Date {
        ...
        guard parts.count == 3,
            ...
            throw CalctlError.usage("invalid date '\(value)'; expected YYYY-MM-DD")
        }
        ...
        guard let date = components.date else {
            throw CalctlError.usage("invalid calendar date '\(value)'")
        }
        return date
    }
    ...
    static func dateTime(_ value: String) throws -> Date {
        ...
        throw CalctlError.usage("invalid date-time '\(value)'; use YYYY-MM-DDTHH:mm or ISO 8601")
    }
...
private final class CalendarStore {
    ...
    func requireFullAccess() throws {
        ...
        guard status == "full-access" else {
            if status == "not-determined" {
                throw CalctlError.operation("calendar access is not determined; run 'calctl authorize'")
            }
            if status == "write-only" {
                throw CalctlError.operation("full calendar access is required; run 'calctl authorize'")
            }
            throw CalctlError.operation("full calendar access is unavailable (\(status)); check System Settings > Privacy & Security > Calendars")
        }
    }
    ...
    func source(_ selector: String) throws -> EKSource {
        ...
        if exact.count > 1 || contains.count > 1 {
            throw CalctlError.operation("source '\(selector)' is ambiguous; use a source ID from 'calctl sources'")
        }
        throw CalctlError.operation("source '\(selector)' not found; run 'calctl sources'")
    }
    ...
    func calendar(_ selector: String, sourceSelector: String?, writable: Bool = false) throws -> EKCalendar {
        ...
        if matches.isEmpty {
            throw CalctlError.operation("calendar '\(selector)' not found or not writable; run 'calctl calendars'")
        }
        throw CalctlError.operation("calendar '\(selector)' is ambiguous; add --source or use a calendar ID")
    }

    func event(_ identifier: String) throws -> EKEvent {
        guard let event = store.event(withIdentifier: identifier) else {
            throw CalctlError.operation("event '\(identifier)' not found; search again because occurrence IDs can change after edits")
        }
        return event
    }
}
...
private func parseColor(_ value: String) throws -> CGColor {
    ...
    guard hex.count == 6, let number = UInt64(hex, radix: 16) else {
        throw CalctlError.usage("invalid color '\(value)'; expected #RRGGBB")
    }
    return CGColor(
...
private func parsePositiveInt(_ value: String?, default defaultValue: Int, name: String) throws -> Int {
    ...
    guard let number = Int(value), number > 0 else {
        throw CalctlError.usage("\(name) must be a positive integer")
    }
    return number
}
...
private func parseURL(_ value: String) throws -> URL {
    guard let url = URL(string: value), url.scheme != nil else {
        throw CalctlError.usage("invalid --url; expected an absolute URL")
    }
    return url
}
...
private func recurrenceRule(args: inout Arguments) throws -> EKRecurrenceRule? {
    ...
    guard untilValue == nil || countValue == nil else {
        throw CalctlError.usage("use only one of --until or --count")
    }
    ...
    default:
        throw CalctlError.usage("--repeat must be daily, weekly, monthly, or yearly")
    }
...
private func replaceRecurrenceEnd(
    ...
    guard rules.count == 1, let rule = rules.first else {
        if rules.isEmpty {
            throw CalctlError.usage("--until and --count require a recurring event")
        }
        throw CalctlError.operation("events with multiple recurrence rules cannot be edited safely")
    }
    ...
    if let untilValue {
        ...
        guard endDate >= event.startDate else {
            throw CalctlError.usage("--until must be the same as or after the selected event date")
        }
...
private func span(for event: EKEvent, value: String?) throws -> EKSpan {
    ...
    if recurring && value == nil {
        throw CalctlError.usage("recurring event requires --span this or --span future")
    }
    switch value?.lowercased() ?? "this" {
    case "this": return .thisEvent
    case "future": return .futureEvents
    default: throw CalctlError.usage("--span must be this or future")
    }
}

private let usage = """
calctl \(calctlVersion) — Apple Calendar EventKit CLI

Usage:
  calctl doctor [--json]
  calctl authorize [--json]
  calctl sources [--json]
  calctl calendars [--source NAME_OR_ID] [--json]
  calctl calendar-create NAME --source NAME_OR_ID [--color #RRGGBB] [--dry-run] [--json]
  calctl calendar-edit NAME_OR_ID [--source NAME_OR_ID] [--title TEXT]
      [--color #RRGGBB] [--dry-run] [--json]
  calctl calendar-delete NAME_OR_ID [--source NAME_OR_ID] --force [--json]
  calctl add TITLE --calendar NAME_OR_ID [--source NAME_OR_ID]
  ...
  calctl events [--calendar NAME_OR_ID] [--source NAME_OR_ID]
      [--from YYYY-MM-DD] [--to YYYY-MM-DD] [--query TEXT] [--limit N] [--json]
  calctl show EVENT_ID [--json]
  calctl edit EVENT_ID [--title TEXT] [--notes TEXT | --clear-notes]
      [--url URL | --clear-url]
      [--calendar NAME_OR_ID] [--source NAME_OR_ID] [--span this|future]
      [--until YYYY-MM-DD | --count N]
      [--dry-run] [--json]
  calctl move EVENT_ID --calendar NAME_OR_ID [--source NAME_OR_ID]
      [--span this|future] [--dry-run] [--json]
  calctl delete EVENT_ID [--span this|future] --force [--json]
  calctl --version
"""

@main
private struct Calctl {
        ...
        do {
            ...
            case "calendar-create":
                ...
                guard let sourceSelector = try args.option("--source") else {
                    throw CalctlError.usage("calendar-create requires --source to prevent creating in the wrong account")
                }
                ...
                guard !duplicate else {
                    throw CalctlError.operation("calendar '\(title)' already exists in source '\(source.title)'")
                }
            ...
            case "calendar-edit":
                ...
                guard title != nil || colorValue != nil else {
                    throw CalctlError.usage("calendar-edit requires --title or --color")
                }
                ...
                if let title {
                    ...
                    guard !duplicate else {
                        throw CalctlError.operation("calendar '\(title)' already exists in source '\(calendar.source.title)'")
                    }
                    calendar.title = title
                }
                if let colorValue { calendar.cgColor = try parseColor(colorValue) }
            ...
            case "calendar-delete":
                ...
                try args.finish()
                guard force else {
                    throw CalctlError.usage("calendar-delete requires --force after reviewing the calendar and its events")
                }
                let calendar = try calendarStore.calendar(selector, sourceSelector: sourceSelector, writable: true)
            ...
            case "add":
                ...
                if let dayValue {
                    ...
                    if event.endDate < event.startDate { throw CalctlError.usage("--end must be the same as or after --date") }
                    ...
                    if event.endDate <= event.startDate { throw CalctlError.usage("--end must be after --start") }
                }
            ...
            case "events":
                ...
                guard end > start else { throw CalctlError.usage("--to must be after --from") }
            ...
            case "edit":
                ...
                guard title != nil || notes != nil || clearNotes || urlValue != nil || clearURL || calendarSelector != nil || untilValue != nil || countValue != nil else {
                    throw CalctlError.usage("edit requires --title, --notes, --clear-notes, --url, --clear-url, --calendar, --until, or --count")
                }
                guard !(notes != nil && clearNotes) else {
                    throw CalctlError.usage("use only one of --notes or --clear-notes")
                }
                guard !(urlValue != nil && clearURL) else {
                    throw CalctlError.usage("use only one of --url or --clear-url")
                }
            ...
            case "move":
                ...
                let identifier = try args.positional("event ID")
                guard let calendarSelector = try args.option("--calendar") else {
                    throw CalctlError.usage("move requires --calendar")
                }
                let sourceSelector = try args.option("--source")
                let spanValue = try args.option("--span")
                let dryRun = args.flag("--dry-run")
            ...
            case "delete":
                ...
                let force = args.flag("--force")
                try args.finish()
                guard force else { throw CalctlError.usage("delete requires --force after reviewing 'calctl show <event-id>'") }
                let event = try calendarStore.event(identifier)
                let selectedSpan = try span(for: event, value: spanValue)
            ...
            default:
                throw CalctlError.usage("unknown command '\(command)'\n\n\(usage)")
            }
