...
description: macOS의 KakaoTalk·iMessage 기록을 읽기 전용 동기화하고 소유자 전용 로컬 SQLite에서 증분 분석하며, 사용자 확인을 거친 KakaoTalk 텍스트 답장만 별도 UI 자동화 경계로 전송한다. "카톡/문자 읽어줘", "새 메시지만 분석", "카톡에 답장해줘", "이 내용 보내줘", "대화 동기화", "언제까지 봤지", "스레드별 일정 분석", "CCT" 요청에 사용한다. iMessage 전송은 지원하지 않는다.
---
...
- KakaoTalk 답장 요청이면 [KakaoTalk 소스](references/kakaotalk.md)의 승인형 답장 절차를 반드시 읽는다.

## 실행 위치

- 이 `SKILL.md`가 있는 디렉터리에서 `../..`를 플러그인 루트로 해석한다. marketplace 캐시 밖 경로를 가정하지 않는다.
- 관리 런타임 경로를 먼저 사용하고, 없을 때만 `PATH`를 확인한다.

  ```bash
  SHERPA_BIN="${SHERPA_INSTALL_ROOT:-$HOME/.local}/bin"
  MSGPIPE="$SHERPA_BIN/msgpipe"
  test -x "$MSGPIPE" || MSGPIPE="$(command -v msgpipe || true)"
  ```

- 플러그인 루트에서 `scripts/doctor.sh messages`를 실행한다. 관리 런타임이 없거나 버전이 다르면 설치 내용을 알린 뒤 Messages 런타임만 설치한다.

  ```bash
  bash <plugin-root>/scripts/install-runtime.sh messages
  ```
...
- KakaoTalk·Messages 원본 DB와 동기화 경로는 계속 읽기 전용이다. `sync`가 쓰는 대상은 별도의 msgpipe 로컬 SQLite뿐이다.
- KakaoTalk 텍스트 전송은 원본 DB 경로와 분리된 `scripts/kakao-reply.py`만 사용한다. iMessage 전송, 파일·이미지 전송, 반응, 읽음 처리는 지원하지 않는다.
...
- 원문 보관소에는 자동 만료가 없다. 제거 요청이면 먼저 `$MSGPIPE state-path`로 정확한 경로를 보여주고 사용자 확인을 받은 뒤에만 `$MSGPIPE purge --force`를 실행한다. 이 명령은 보관 원문과 분석 이력을 함께 삭제하며 플러그인 제거만으로는 실행되지 않는다.
...
- 각 요청에서 `START_DATE`와 `END_DATE`를 새로 계산한다. 문서 작성일이나 이전 실행의 고정 날짜를 재사용하지 않는다.
- `TIMEZONE`은 Sherpa 로컬 설정의 `timezone`, 없으면 사용자의 현재 IANA 시간대를 사용한다. 알 수 없으면 날짜 경계가 달라질 수 있으므로 동기화 전에 묻는다.
...
4. 모델 입력에는 CCT 범례, 최신 전역 rollup, 해당 스레드의 최신 rollup, 이번 pending CCT만 넣는다. 호스트에서 사용 중인 모델과 reasoning effort를 `ANALYSIS_MODEL`, `REASONING_EFFORT`에 기록하고, 식별할 수 없으면 둘 다 `host-selected`로 기록한다. 특정 호스트 모델을 강제하지 않는다. 스레드마다 새 분석 컨텍스트를 사용한다.
...
6. 분석이 성공한 뒤 각 CCT `S` 세션을 별도로 저장한다. `--start`는 해당 `S`, `--end`는 다음 `S`의 시각이며 마지막 세션은 요청 범위의 끝이다. 이 커밋은 그 구간에서 실제로 CCT로 제시된 pending 메시지만 분석 완료로 연결한다.
   ...
   printf '%s' '<session summary>' | "$MSGPIPE" context put session \
     --thread K001 --start "$SESSION_START" --end "$SESSION_END" \
     --timezone "$TIMEZONE" \
