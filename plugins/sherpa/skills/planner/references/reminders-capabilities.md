# Apple Reminders 기능 매트릭스

이 문서는 미리 알림 자동화 도구를 고르는 기준을 정리한다. macOS 업데이트로 사설 인터페이스의 동작이 달라질 수 있으므로 실제 변경 전에는 Sherpa의 `scripts/doctor.sh planner`와 좁은 범위의 읽기 작업으로 다시 확인한다. RemCTL doctor 원문은 로컬 경로와 리스트 정보를 포함하므로 모델 출력으로 전달하지 않는다.
