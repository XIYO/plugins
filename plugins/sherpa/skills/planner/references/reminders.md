...
# 읽기
$SHERPA planner reminders lists                          # 트리(그룹·섹션·개수)
$SHERPA planner reminders lists --json                   # 구조(id·counts·children·objectUUID)
$SHERPA planner reminders show "리스트"                   # 미완료만(기본)
$SHERPA planner reminders show "리스트" --completed       # 완료 포함
$SHERPA planner reminders show "리스트" --completed --json
$SHERPA planner reminders info <id>                      # 단건 상세(List·Section·Status·Deep link)
$SHERPA planner reminders sections                       # 전체 섹션
$SHERPA planner reminders subtasks <id>                  # 하위작업
$SHERPA planner reminders stats                          # Total/Active/Completed/Lists/Sections
$SHERPA planner reminders export -l "리스트" --format json|csv
# 항목
$SHERPA planner reminders add "제목" -l "리스트" [-d "2026-12-28 10:00"] [-p high] [-f]
$SHERPA planner reminders add "제목" -l "리스트" --private --section "섹션" --subtask "a" -t "Food"
$SHERPA planner reminders edit <id> --title "새 제목"
$SHERPA planner reminders edit <id> --list "리스트"                 # 이동(경계 넘으면 clone-delete→새 id 반환)
$SHERPA planner reminders edit <id> -d 2026-12-28    # 날짜만=하루종일 / "…HH:MM"=시간지정 / clear=제거
$SHERPA planner reminders edit <id> --private --section "섹션"       # 섹션 배정
$SHERPA planner reminders edit <id> --private --subtask "스텝"       # 하위작업 추가
$SHERPA planner reminders edit <id> --private -t "Food,Travel"       # 태그 추가(--set-tags/--clear-tags/--remove-tag)
$SHERPA planner reminders edit <id> --private --flagged              # 진짜 플래그
$SHERPA planner reminders done <id> / $SHERPA planner reminders undone <id> / $SHERPA planner reminders flag <id> / $SHERPA planner reminders delete <id> --force
# 구조
$SHERPA planner reminders list-create "이름" [--color …]
$SHERPA planner reminders list-rename … / $SHERPA planner reminders list-delete --list-id <n> --force
$SHERPA planner reminders section-create "이름" -l "리스트" --private
$SHERPA planner reminders section-rename / $SHERPA planner reminders section-delete
$SHERPA planner reminders group-create / $SHERPA planner reminders group-edit / $SHERPA planner reminders group-delete --group-id <n> --private --force
```
...
```bash
newid=$($SHERPA planner reminders edit <id> --list "대상 리스트" --json | python3 -c "import sys,json;print(json.load(sys.stdin)['id'])")
$SHERPA planner reminders edit "$newid" --private --section "대상 섹션"
```
