...
SQLite 애플리케이션 암호화는 이번 결정에 포함하지 않는다. 새 상태 디렉터리는 `0700`, 파일은 `0600`으로 만들고 기존 공유 디렉터리는 권한을 바꾸지 않은 채 거부한다. FileVault 사용을 권장하며 원문을 로그·Git·보조 덤프에 복제하지 않는다. 원문은 자동 만료하지 않고, 전체 보관소 경로를 확인한 뒤 `purge --force`를 실행하는 명시적 수명주기로 삭제한다.
