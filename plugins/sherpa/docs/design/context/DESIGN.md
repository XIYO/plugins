...
| 엔터티 | 핵심 필드 | 비고 |
|---|---|---|
| `NormalizedMessage` | source, message_id, conversation, author, timestamp_utc, kind, content, attachments | 최적화 전 원문 모델 |
| `source_conversation` | source, source_conversation_id, display_name, alias | 모델에는 alias만 전달 |
| `speaker` | conversation_id, source_author_id, display_name, alias, is_self | 자기 자신은 A |
...
| `analysis_context` | scope, period, model, effort, summary, coverage, rollup link/watermark | append-only 세션/대화/전역 요약 |
...
5. `summary put session`은 제시된 pending 행의 정렬된 `(message_id, content_sha256)` 집합 해시와 요약을 저장하고 분석 연결을 한 트랜잭션에서 설정한다.
6. `summary inputs thread`는 미반영 세션 요약을 모두 CTX로 내보낸다. `summary inputs global`은 별칭별 최신 미반영 대화 rollup만 내보내 중간 누적 버전을 생략한다.
7. 분석기는 기존 대화 rollup과 CTX 세션 입력으로 새 `thread` rollup을, 기존 전역 rollup과 CTX 대화 입력으로 새 `global` rollup을 만든다.
8. `summary put thread|global --through-context-id`는 rollup insert와 watermark 이하 입력 연결을 원자적으로 처리한다. 출력 뒤 생긴 더 큰 ID는 다음 실행에 남는다.
9. 다음 분석은 최신 전역/대화 rollup과 새 pending만 사용한다.
...
- 새 상태 디렉터리/파일 권한을 `0700`/`0600`으로 만들고 기존 공유 디렉터리는 권한 변경 없이 거부한다.
- SQLite 자체는 암호화되지 않으므로 FileVault 사용을 권장하고, 전체 보관소 삭제는 경로 확인 뒤 `purge --force`로만 수행한다.

## 테스트 전략

합성 fixture로 소스별 파싱, 원문 round-trip, 동기화 멱등성, 수정 시 pending 복귀, 제시 전 요약 거부, 세션 요약의 원자적 분석 연결, CCT escaping, 별칭 결정성, 공유 상위 폴더 거부와 확인형 purge 범위를 검증한다. 실데이터 테스트는 집계만 출력한다.

## 관련 문서

**요구사항** — [Context Engine Requirements](../../requirements/context/README.md)

**유스케이스** — [UC-CTX-001](../../requirements/context/use-cases/UC-CTX-001-export-conversation-context.md)
