...
#[command(
    name = "sherpa context",
    about = "Collect, normalize, and incrementally review personal context"
)]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
pub fn run_from(arguments: impl IntoIterator<Item = OsString>) -> Result<()> {
    let cli =
        Cli::try_parse_from(std::iter::once(OsString::from("sherpa context")).chain(arguments))?;
    run(cli)
}
...
fn render_rollup_inputs(
    scope: &ContextScope,
    contexts: &[crate::state::AnalysisContext],
) -> String {
...
struct PreparedPipeline {
    timezone: Tz,
    messages: Vec<crate::NormalizedMessage>,
}
