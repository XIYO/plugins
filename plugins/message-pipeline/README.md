...
> **호환 패키지:** 신규 사용자는 이 메시지 스킬과 런타임을 포함한 `sherpa@xiyo`를 설치한다. 기존 설치를 유지할 때만 이 패키지를 사용하고 두 패키지를 동시에 활성화하지 않는다.
...
- 동기화한 원문은 소유자 전용 로컬 SQLite에 명시적으로 비울 때까지 저장하며, 파일 자체는 애플리케이션 수준에서 암호화하지 않는다. FileVault 사용을 강하게 권장한다.
...
v0.2.1은 읽기 전용 소스 어댑터, 원문 아카이브, 멱등 동기화, 공통 옵티마이저, CCT, 토큰 벤치마크, 증분 분석 상태와 확인형 보관소 삭제를 구현한다.
...
printf '%s' '<session summary>' | msgpipe context put session \
  --thread K001 \
  --start "$SESSION_START" --end "$SESSION_END" --timezone "$TIMEZONE" \
...
기본 상태 경로는 운영체제의 XIYO/msgpipe 로컬 앱 데이터 디렉터리 아래 `state.sqlite3`다. 새 전용 디렉터리는 `0700`, 파일은 `0600`으로 만들고, 기존 공유 디렉터리는 권한을 임의로 바꾸지 않고 거부한다. SQLite는 애플리케이션 수준에서 암호화하지 않으므로 FileVault 사용을 권장하며, 복사와 백업에도 채팅 원문이 포함된다고 취급한다.

원문에는 자동 보존 기한이 없다. 전체 원문 보관소와 분석 이력을 삭제하려면 먼저 정확한 경로를 확인하고 명시적으로 실행한다.
...
`purge --force`는 사용자 확인 뒤에만 실행한다. 선택한 데이터베이스와 SQLite `-wal`·`-shm`·rollback `-journal` 파일만 삭제하고 같은 폴더의 다른 파일은 건드리지 않는다. 플러그인을 제거해도 이 데이터는 자동으로 삭제되지 않는다.
