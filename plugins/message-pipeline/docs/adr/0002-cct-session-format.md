...
모델 입력 기본 형식은 버전이 있는 CCT3로 한다. 스레드 `T`, 날짜 `D`, 세션 시작 `S`, 화자/본문 행을 사용하고 빈 화자는 직전 화자를 상속한다. `schedule` 기본 세션 간격은 30분이며 `exact`는 CCT2 분 단위 행을 사용한다. 실제 이름과 정확한 시각은 중앙 상태에 남긴다.
