...
이 저장소는 Codex와 Claude Code가 설치할 수 있는 XIYO 공개 플러그인 marketplace다. 소비자용 정식 제품은 `sherpa@xiyo` 하나이며 Context·Planner 전문 스킬과 런타임을 내부에 함께 둔다. 플러그인 설치기는 디렉터리를 캐시로 복사하므로 저장소 밖 파일이나 절대 경로에 의존하지 않는다.
...
- `catalog-policy.json`: 공개할 단일 정식 플러그인 선언
...
Sherpa는 `skills/sherpa`, `skills/context`, `skills/planner`를 포함한다. 플러그인끼리 의존하지 않는다. 설치본은 항상 자기완결형이어야 하며 개인 `x-*` 스킬, 저장소 밖 상대경로, 작성자 컴퓨터의 절대경로에 의존하지 않는다.

독립 호환 플러그인과 공개 보조 실행 파일을 만들지 않는다. Context와 Planner의 Rust 코드는 `sherpa` 프로세스에 라이브러리로 연결하고, 운영체제 경계 어댑터만 별도 실행 파일로 둔다.
...
- 원문 대화는 Context의 소유자 전용 로컬 SQLite에만 저장한다. 동일 원본 메시지는 멱등 upsert하고, 수정된 메시지는 기존 분석 연결을 끊어 다시 pending으로 만든다.
...
- Sherpa 플러그인 버전과 내부 컴포넌트 버전은 독립적이다. `runtime-versions.json`이 앱·도메인 라이브러리·플랫폼 어댑터 고정 버전의 정본이다.
