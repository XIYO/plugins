...
`apple-calendar`와 `message-pipeline` 독립 패키지는 마이그레이션 기간의 호환 경로다. 공유 런타임 소스만 `scripts/check-legacy-sync.py`로 검사하고, 도메인 문서와 신규 기능의 정본은 Sherpa만 사용한다.
...
- Sherpa 플러그인 버전과 내부 런타임 버전은 독립적이다. `runtime-versions.json`이 통합 `sherpa` CLI와 호환·플랫폼 어댑터 고정 버전의 정본이다.
